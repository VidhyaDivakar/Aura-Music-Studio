...
let sampleVoices = [];
let isRecording = false, recStart = 0, totalPausedTime = 0, recData = [], activePlaybackId = null, recInterval = null;
...
if (pianoRef) {
    for (let i = 0; i < 64; i++) {
        ...
        const computerKey = revMap[midi];
        const key = document.createElement('div');
        key.className = `key ${label.includes('#') ? 'black' : 'white'}`;
        key.innerHTML = `<span>${label}</span>${computerKey ? `<div class="key-hint">${computerKey.toUpperCase()}</div>` : ''}`;
...
async function analyzeWithAI(id, midiArray) {
    ...
    const res = await callGemini(`Producer mode. Notes: [${names}]. If famous song, name it. Otherwise give a trendy name and mood. Format Name: [Name] | Analysis: [Analysis]`);
    if (res && res.includes('|')) {
        const [n, a] = res.split('|');
        const trendyName = n.replace('Name:', '').trim(), analysis = a.replace('Analysis:', '').trim();
        document.getElementById(`ai-report-${id}`).innerText = analysis;
        const saved = JSON.parse(localStorage.getItem('sb_pro_v4') || '[]');
...
async function generateAITone() {
    ...
    try { const motif = JSON.parse(res.match(/\[.*\]/)[0]); playAsset(motif, 3, "ai_gen"); } catch (e) { alert("AI error. Try again!"); }
}
...
function renderUser() {
    ...
    saved.forEach(mix => {
        ...
        const card = document.createElement('div'); card.className = 'tone-card';
        card.innerHTML = `<div class="card-top"><div><h4>${mix.name}</h4><small id="ai-report-${mix.id}" style="color:var(--studio-blue)">${mix.aiReport || 'No AI analysis yet.'}</small></div></div>
...
function playAsset(motif, dur) {
    if (audioCtx.state === 'suspended') audioCtx.resume();
    motif.forEach((v, i) => setTimeout(() => { const s = playNote(55 + v, true, 0.5); const k = document.querySelector(`[data-midi="${55 + v}"]`); k?.classList.add('active'); setTimeout(() => k?.classList.remove('active'), 150); }, i * 200));
}
